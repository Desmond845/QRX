...
async function download() {
  ...
  if (window.trackQRX) {
    trackQRX('qr_downloaded', {
      ...
      'label': 'png_download',
      'value': downloadCount,
      'current_qr_text': document.getElementById("input").value.trim().substring(0, 50) || "QRX"
    });
  }
...
function showCameraError(error) {
  ...
  if (error.name === 'NotFoundError' || error.name === 'DevicesNotFoundError') {
    ...
    errorMessage = "Camera access denied<br>Please allow camera permission and refresh";
  } else if (error.name === 'NotReadableError' || err.name === 'TrackStartError') {
  ...
  if (qrInput) {
    ...
    qrInput.addEventListener('change', () => {
      if (qrInput.value && qrInput.value !== lastGenerated) {
        ...
        // Track content type
        const isURL = qrInput.value.includes('http') || 
...
setInterval(function() {
  ...
  if (minutesOnSite > 0 && minutesOnSite <= 30) { // Track up to 10 minutes
