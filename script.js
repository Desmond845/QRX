...
function showSection(sectionName) {
  ...
  if(currentActive && currentActive.id === sectionName + "Section") {
    console.log(`Already showing ${sectionName}`);
    return;
  }
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            ${message.includes("https://") || message.includes("http://") ? `
              <p>URL Detected</p>
              <button class='url-button' onclick='openUrl("${message}")'>Open URL</button>` : ""}
        </div>
    </div>
    `;
...
if ("serviceWorker" in navigator) {
  // Register service worker
   const swPath = window.location.pathname.includes('/QRX/') 
      ? './sw.js' 
      : './sw.js';
  // navigator.serviceWorker.register("/sw.js");

    navigator.serviceWorker.register('/QRX/sw.js', { scope: '/QRX/' })
      .then(registration => {
...
function setupInstallPrompt() {
  ...
  window.addEventListener('appinstalled', () => {
    ...
    toast.success("QRX installed! 🎉", 3000);
  });
}
